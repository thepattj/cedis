<?php
    ...
    $nombrehoja = 'BVG BKBQA '.$nf;


	$sqlventas16 = "SELECT ROUND(SUM(((detalle.precio*detalle.cantidad)-(detalle.descuento/100)*(detalle.precio*detalle.cantidad))/((detalle.impuesto1*0.01)+1)),2,1) as TOTAL, gpo.descripcion    
                  ...
                  WHERE tick.cierre BETWEEN '".$fechadr." 12:00:00 AM' AND '".$fechadr." 11:59:59 PM' AND tick.pagado = 1 and tick.cancelado =0 AND detalle.impuesto1 > 0
                  GROUP BY gpo.descripcion
                  ORDER BY gpo.descripcion ASC";

    $sqlventas8 = "SELECT ROUND(SUM(((detalle.precio*detalle.cantidad)-(detalle.descuento/100)*(detalle.precio*detalle.cantidad))/((detalle.impuesto2*0.01)+1)),2,1) as TOTAL, gpo.descripcion    
                   FROM cheqdet detalle LEFT JOIN cheques tick ON detalle.foliodet = tick.folio LEFT JOIN productos prod ON detalle.idproducto = prod.idproducto RIGHT JOIN grupos gpo ON prod.idgrupo = gpo.idgrupo
                   WHERE tick.cierre BETWEEN '".$fechadr." 12:00:00 AM' AND '".$fechadr." 11:59:59 PM' AND tick.pagado = 1 and tick.cancelado =0 AND detalle.impuesto2 > 0 AND detalle.impuesto1 = 0
                   GROUP BY gpo.descripcion
                   ORDER BY gpo.descripcion ASC";
	...
	$stmt = sqlsrv_query($con, $sqlventas16);
    $stmt3 = sqlsrv_query($con, $sqlventas8);
    ...
    if( $stmt3 === false ) {
        echo "Error in executing statement3.\n";  
        die( print_r( sqlsrv_errors(), true)); 
        echo "ERROR DE DATOS";
    }else{
	...
	if( $stmt1 === false ) {
    	echo "Error in executing statement1.\n";  
	    die( print_r( sqlsrv_errors(), true)); 
	    echo "ERROR DE DATOS";
	}else{
    ...
    if( $stmt2 === false ) {
        echo "Error in executing statement2.\n";  
        die( print_r( sqlsrv_errors(), true)); 
        echo "ERROR DE DATOS";
    }else{
    ...
    $objPHPExcel->getProperties()->setCreator("BK"); // Nombre del autor
    ...
    /*FOR QUE ESCRIBE EL VALOR QUE TRAE EL OBJ DE VENTA de 8%*/
    $l=0;   
    for($k=11; ; $k++){
        $celda1 = "A".$k;
        $celda7 = "G".$k;
    ...
    /*FOR QUE INSERTA EL OBJ DE venta de 16% */
    $m=0;   
    for($q=$suma; ; $q++){
        $celda11 = "A".$q;
        $celda71 = "G".$q;
        ...
        if($q< $f+$q){
            //echo "celda".$celda1." ".$folio[$l];
            $objPHPExcel->getActiveSheet()->SetCellValue($celda11, $Descripcion2[$m]);
            $objPHPExcel->getActiveSheet()->SetCellValue($celda71, $Importe2[$m]);            
            /*$objPHPExcel->getActiveSheet()->SetCellValue($celda8, $nombre[$l]);*/
            if($m < $f-1 ){ $m=$m+1; }else{ break; }
        }else{ break; }
    }
    ...
    /*FOR QUE INSERTA EL OBJ DE COSTOS */
    ...
    header('Content-Disposition: attachment;filename="BVG BKBQA'.$nf.'.xlsx"');
