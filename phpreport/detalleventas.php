<?php
    ...
    $fechahoja = date("d-m-Y");

    $nombrehoja = 'Reporte del dia'.$fechahoja;

    //die( print_r($nsrvr." ".$fechaa." ".$con));
    //echo $fechaa.$fechab;
    $sql = "SELECT  dbo.cheqdet.foliodet as folio, dbo.cheques.idcliente, dbo.clientes.nombre, dbo.cheqdet.idproducto, dbo.productos.descripcion, dbo.cheqdet.precio AS costo, dbo.cheqdet.cantidad, dbo.cheqdet.impuesto1 AS IVA, (dbo.cheqdet.precio*dbo.cheqdet.cantidad) AS TOTAL, dbo.cheques.numcheque, dbo.cheques.cierre, dbo.cheques.fecha
            ...
            WHERE dbo.cheques.cancelado = 0 AND dbo.cheques.cierre BETWEEN '".$fechaa." 12:00:00 AM' AND '".$fechab." 11:59:59 PM' ORDER BY dbo.cheques.idcliente ASC, dbo.cheques.cierre ASC";
    ...
    if( $stmt === false ) {
        ...
        //for($j=0; $j<$i; $j++){ /*echo $cliente[$j]."\n"; echo "\t================================\t";*/ if ($cliente[$j] == "BRALM") { echo $cliente[$j]."\n"; echo "\n================================\n"; }   }
    }

   //for($a=0; $a<$i; $a++){ if ($cliente[$a] == "BRALN" || $nombre[$a] =="BRIOCHE BRASSIERE ALM") { echo "salio!"; }    }
    ...
    header('Content-Disposition: attachment;filename="DetalleVentas'.$fechahoja.'.xlsx"');
