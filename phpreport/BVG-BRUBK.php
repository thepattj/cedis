<?php
    ...
    $nombrehoja = 'BVG BRUBK '.$nf;


	$sqlventas = "SELECT ROUND(SUM((detalle.precio * detalle.cantidad - (detalle.descuento / 100) *(detalle.precio * detalle.cantidad))/1.16),2,1) as TOTAL, gpo.descripcion    
				  ...
				  WHERE tick.fecha BETWEEN '".$fechadr." 12:00:00 AM' AND '".$fechadr." 11:59:59 PM' AND tick.pagado = 1 and tick.cancelado =0
...
/*=========================================================CREACION DEL EXCEL====================================================================*/
    ...
    $objPHPExcel->getProperties()->setCreator("BRUBK"); // Nombre del autor
    ...
    /*FOR QUE INSERTA EL OBJ DE COSTOS DE*/
    $n=0;   
    for($p=$inicioc; ; $p++){
        $celda11 = "A".$p;
        $celda71 = "G".$p;
    ...
    header('Content-Disposition: attachment;filename="BVG BRUBK'.$nf.'.xlsx"');
